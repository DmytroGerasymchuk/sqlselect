...
namespace libtoken
{
	...
	{
		if (!state)
			throw libtoken_exception("this token_stream is not readable any more", line_buf);

		if (token_buf.size() > 0) // if there are some previously buffered tokens
		{ // then return the last one and make buffer one position shorter
			t = token_buf[token_buf.size() - 1];
			token_buf.pop_back();
		}
		else
		{
				...
				if (c == line_buf.eol_signal())
				{
					if (something_read(t, tmp)) // if already something has been read
					{
						if (settings.newline_as_token)
							line_buf.ungetch(); // put EOL back to be read later
						break;
					}
